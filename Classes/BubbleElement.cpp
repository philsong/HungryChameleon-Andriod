...
{
	m_Texture = CCFileUtils::sharedFileUtils()->fullPathForFilename("bubbles.png");
...
bool BubbleElement::LoadBubbles()
{
	...
	std::string fullname = CCFileUtils::sharedFileUtils()->fullPathForFilename("spiders2.xml");

	CCLOG("BubbleElement begin LoadBubbles = %s", fullname.c_str());

    pBuffer = CCFileUtils::sharedFileUtils()->getFileData("spiders2.xml", "r", &bufferSize);
    ...
    if (pBuffer)
    {
		...
		if (!root)
		{
			CCLOG("NULL root %s", fullname.c_str());
			return false;
		}
		...
		m_Texture = CCFileUtils::sharedFileUtils()->fullPathForFilename(xsettings->Attribute("texture"));
		CCLOG("BubbleElement m_Texture = %s", m_Texture.c_str());
		...
		CCLOG("BubbleElement end LoadBubbles = %s", fullname.c_str());
		return true;
    }
